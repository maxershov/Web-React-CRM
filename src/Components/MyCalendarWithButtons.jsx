...
const MyCalendarWithButtons = (props) => {
  ...
  return (<>
    <div className={props.dateType + "Field"}>
      ...
      <input type="text" readOnly={true} value={props.date} />
      <button type="button" onClick={() => setRenderCalendar('block')}>Выбрать новую дату</button>
    </div><div style={{display:renderCalendar}} className="calendar" id="calendar">
      <Calendar className="calendar" onChange={date => changeDate(date)}></Calendar>
